...
slider_open.addEventListener("touchstart", (e) => touch_open_start_x = e.changedTouches[0].clientX);
...
function slider_open_sidebar() {
    ...
    if (touch_open_end_x < touch_open_start_x) {
        ...
        if (touch_open_start_x > window.innerWidth - 10) open_close_sidebar();
    }
};
...
function slider_close_sidebar() {
    ...
    if (touch_close_end_x == touch_close_start_x && touch_close_end_x == touch_close_start_x) {
        ...
        if (touch_close_start_x > window.innerWidth - 10) open_close_sidebar();
    }
}
